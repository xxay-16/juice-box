...
2. CPU Gate：raw observed 的 R32F Tile 与 PNG 参考导出已建立；Observed、dense Expected、
   ...
   Gate。该 production Gate 现覆盖 35 个模式：原 11 个标准模式，OEP1/OEP1V2、
   ...
   OERATIOMINUS/OERATIOMINUSP1。OME/CME 目前只作为 OCMEVS 的内部 source type，
   ...
   `Production pixel-grid match: ... (35 modes, 36 ordered cells per mode)` 与
