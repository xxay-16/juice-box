...
2. CPU Gate：raw observed 的 R32F Tile 与 PNG 参考导出已建立；Observed、dense Expected、
   ...
   Gate。该 production Gate 现覆盖 23 个模式：原 11 个标准模式，OEP1/OEP1V2、
   OECTRLP1/OECTRLP1V2、OEVSP1/OEVSP1V2、LOGEO、LOGCEO，以及 EXPLOGEO、
   EXPLOGCEO、OCMEVS、DIFF。OME/CME 目前只作为 OCMEVS 的内部 source type，
   ...
   `Production pixel-grid match: ... (23 modes, 36 ordered cells per mode)` 与
