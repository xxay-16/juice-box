...
> 动态热图、NONE/KR/VC/VC_SQRT、Observed/Expected/OE/Pearson、标准 Control/比较/Log 视图、Assembly 重排/debris/分组编辑
...
> production renderer 像素 Gate；这些已验证模式现在可通过 `V` 打开的 Matrix View
> 选择器直接选择。Java 21 字段 saved-state XML 的单 observed/单 control
> 恢复也已接入。独立 OME/CME、NORM/EIGENVECTOR、track/annotation session 恢复和
