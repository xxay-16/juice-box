...
use heatmap_wgpu::comparison::{
    ...
    rasterize_log_expected_ratio_contacts, rasterize_log_ratio_contacts, rasterize_ratio_contacts,
    rasterize_ratio_with_baselines_contacts, scale_for_log_vs, scale_for_vs,
};
use hic_core::{
...
pub enum MatrixType {
    ...
    LogControl,
    LogControlExpected,
    ExpLogControlExpected,
    ControlMinusExpected,
    Vs,
    Ratio,
    RatioV2,
    RatioP1,
...
impl MatrixType {
    ...
    pub const EXPECTED_TRANSFORM_MODES: [Self; 4] = [
    ...
    pub const LOG_COMPARISON_MODES: [Self; 5] = [
        Self::LogVs,
        Self::LogRatio,
        Self::LogRatioV2,
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            Self::ObservedOverExpectedP1 => "(Observed+1)/(Expected+1)",
            Self::ObservedOverExpectedP1V2 => "Log[(Observed+1)/(Expected+1)]",
            Self::Pearson => "Pearson",
            Self::LogObserved => "Log[Observed+1]",
            Self::LogObservedExpected => "Log[Observed+1]/Log[Expected+1]",
            Self::ExpLogObservedExpected => "e^(Log[Observed+1]/Log[Expected+1])",
            Self::ObservedMinusExpected => "Observed-Expected",
            Self::Control => "Control",
            Self::ControlOverExpected => "Control/ExpectedC",
            Self::ControlOverExpectedV2 => "Log[Control/ExpectedC]",
            Self::ControlOverExpectedP1 => "(Control+1)/(ExpectedC+1)",
            Self::ControlOverExpectedP1V2 => "Log[(Control+1)/(ExpectedC+1)]",
            Self::ControlPearson => "Control Pearson",
            Self::LogControl => "Log[Control+1]",
            Self::LogControlExpected => "Log[Control+1]/Log[ExpectedC+1]",
            Self::ExpLogControlExpected => "e^(Log[Control+1]/Log[ExpectedC+1])",
            Self::ControlMinusExpected => "Control-ExpectedC",
            ...
            Self::ObservedOverExpectedVsP1 => {
                "(Observed+1)/(Expected+1) vs (Control+1)/(ExpectedC+1)"
            }
            Self::ObservedOverExpectedVsP1V2 => {
                "Log[(Observed+1)/(Expected+1)] vs Log[(Control+1)/(ExpectedC+1)]"
            }
            Self::ObservedMinusExpectedVs => "Observed-Expected vs Control-Expected",
            Self::LogObservedExpectedVs => {
                "Log[Observed+1]/Log[Expected+1] vs Log[Control+1]/Log[ExpectedC+1]"
            }
            Self::LogVs => "Log[Observed/AvgO+1] vs Log[Control/AvgC+1]",
            Self::LogRatio => "Log[Observed/AvgO+1]/Log[Control/AvgC+1]",
            Self::LogRatioV2 => "Log[Log[Observed/AvgO+1]/Log[Control/AvgC+1]]",
            Self::LogExpectedRatio => {
                "(Log[Observed+1]/Log[Expected+1])/(Log[Control+1]/Log[ExpectedC+1])"
            }
            Self::LogExpectedRatioV2 => {
    ...
    pub fn next(self, control_available: bool) -> Self {
        ...
        match self {
            ...
            | Self::LogRatioV2
            | Self::LogExpectedRatio
            | Self::LogExpectedRatioV2 => Self::Observed,
            Self::ObservedOverExpectedP1
            | Self::ObservedOverExpectedP1V2
            | Self::ControlOverExpectedP1
            | Self::ControlOverExpectedP1V2
            | Self::ObservedOverExpectedVsP1
            | Self::ObservedOverExpectedVsP1V2 => Self::Observed,
        }
    }
...
fn build_tile_streaming(
    ...
    let contact_normalization_vector = matches!(
        ...
        MatrixType::Observed
            ...
            | MatrixType::ObservedOverExpectedP1
            | MatrixType::ObservedOverExpectedP1V2
            | MatrixType::LogObserved
            | MatrixType::LogObservedExpected
            | MatrixType::ExpLogObservedExpected
            | MatrixType::ObservedMinusExpected
            | MatrixType::Control
            | MatrixType::ControlOverExpected
            | MatrixType::ControlOverExpectedV2
            | MatrixType::ControlOverExpectedP1
            | MatrixType::ControlOverExpectedP1V2
            | MatrixType::LogControl
            | MatrixType::LogControlExpected
            | MatrixType::ExpLogControlExpected
...
fn build_comparison_tile(
    ...
    if matches!(
        ...
        let values = if expected_comparison {
            ...
            let observed_expected_at = |bin_x: i32, bin_y: i32| {
                if symmetric {
                    observed_expected
                        .value_for(
                            observed_matrix.chromosome_1,
            ...
            let control_expected_at = |bin_x: i32, bin_y: i32| {
                if symmetric {
                    control_expected
                        .value_for(
                            control_matrix.chromosome_1,
            ...
            if matches!(
                request.matrix_type,
                MatrixType::LogExpectedRatio | MatrixType::LogExpectedRatioV2
            ) {
                rasterize_log_expected_ratio_contacts(
                    &observed_contacts,
                    &control_contacts,
                    bin_bounds,
                    OUTPUT_SIZE,
                    symmetric,
                    observed_expected_at,
                    control_expected_at,
                )
            } else {
            ...
            request.matrix_type,
            MatrixType::LogRatio | MatrixType::LogRatioV2
        ) {
            rasterize_log_ratio_contacts(
                &observed_contacts,
                &control_contacts,
                bin_bounds,
                OUTPUT_SIZE,
    ...
    let values = match request.matrix_type {
        ...
        MatrixType::LogVs => combine_triangles(
            &scale_for_log_vs(
                &observed_values,
                zoom_average_count(observed_file, observed_matrix, observed_zoom),
                zoom_average_count(control_file, control_matrix, control_zoom),
        ...
        | MatrixType::ObservedOverExpectedVsP1
        | MatrixType::ObservedOverExpectedVsP1V2
        | MatrixType::ObservedMinusExpectedVs
...
fn comparison_raster_types(matrix_type: MatrixType) -> (MatrixType, MatrixType) {
    match matrix_type {
        ...
        MatrixType::ObservedOverExpectedVsP1 | MatrixType::ObservedOverExpectedVsP1V2 => (
            MatrixType::ObservedOverExpectedP1,
            MatrixType::ControlOverExpectedP1,
        ),
        MatrixType::LogObservedExpectedVs => (
    ...
    IntensityTile::accumulate_contact_window_dirty(
        ...
        records.iter().filter_map(|record| {
            ...
            let counts = match matrix_type {
                ...
                | MatrixType::ControlOverExpectedV2
                | MatrixType::ControlOverExpectedP1
                | MatrixType::ControlOverExpectedP1V2 => observed_over_expected(
                    ContactRecord {
                        bin_x,
                        bin_y,
                        counts,
                ...
                MatrixType::ExpLogObservedExpected | MatrixType::ExpLogControlExpected => {
                    exp_log_observed_expected(
                        ContactRecord {
                            bin_x,
                            bin_y,
                            counts,
                ...
                MatrixType::ObservedMinusExpected | MatrixType::ControlMinusExpected => {
                    counts - expected_value(expected_vector?, chromosome, bin_x, bin_y)?
                }
...
fn matrix_type_id(matrix_type: MatrixType) -> u32 {
    match matrix_type {
        ...
        MatrixType::ObservedOverExpectedP1 => 20,
        MatrixType::ObservedOverExpectedP1V2 => 21,
        MatrixType::ControlOverExpectedP1 => 22,
        MatrixType::ControlOverExpectedP1V2 => 23,
        MatrixType::ObservedOverExpectedVsP1 => 24,
        MatrixType::ObservedOverExpectedVsP1V2 => 25,
        MatrixType::ExpLogObservedExpected => 26,
        MatrixType::ExpLogControlExpected => 27,
        MatrixType::ObservedMinusExpected => 28,
...
fn observed_over_expected(
    ...
    let score = observed_over_expected_score(record.counts, expected, pseudo_count);
...
fn expected_value(
    ...
    let distance = u64::from((mapped_bin_x - mapped_bin_y).unsigned_abs());
    let value = expected.value_for(chromosome, distance)? as f32;
...
fn exp_log_observed_expected(
    ...
    let distance = u64::from((mapped_bin_x - mapped_bin_y).unsigned_abs());
    let expected = expected.value_for(chromosome, distance)? as f32;
    // Match Java's Math.exp(Math.log(float + 1) / Math.log(float + 1)):
    // both additions round as float, then every transcendental and the
    // division remain double until the final cast.
    let observed_plus_one = record.counts + 1.0;
    let expected_plus_one = expected + 1.0;
    let score =
        (f64::from(observed_plus_one).ln() / f64::from(expected_plus_one).ln()).exp() as f32;
    score.is_finite().then_some(score)
}
...
mod tests {
    ...
    fn observed_over_expected_uses_mapped_assembly_distance() {
        ...
        assert_eq!(
            observed_over_expected(source_record, &expected, 1, 4, 5, 0.0),
            Some(4.0)
        );
    }
    ...
    #[test]
    fn pseudocount_observed_expected_uses_java_plus_one_formula() {
        let expected = ExpectedValueVector {
            key: ExpectedValueKey {
                normalization: "NONE".to_owned(),
                unit: MatrixUnit::BasePairs,
            ...
            values: vec![4.0],
            chromosome_factors: std::collections::BTreeMap::from([(1, 1.0)]),
        };
        let record = ContactRecord {
            bin_x: 0,
            bin_y: 0,
        ...
        assert_eq!(
            observed_over_expected(record, &expected, 1, 0, 0, 1.0),
            Some(2.0)
        );
    }

    #[test]
    fn pseudocount_v2_modes_keep_raw_values_and_use_log_ratio_color() {
        assert!(MatrixType::ObservedOverExpectedP1V2.uses_log_ratio_color_scale());
        assert!(MatrixType::ControlOverExpectedP1V2.uses_log_ratio_color_scale());
        assert!(MatrixType::ObservedOverExpectedVsP1V2.uses_log_ratio_color_scale());
        assert!(!MatrixType::ObservedOverExpectedP1.uses_log_ratio_color_scale());
        assert_eq!(
            comparison_raster_types(MatrixType::ObservedOverExpectedVsP1V2),
            (
                MatrixType::ObservedOverExpectedP1,
                MatrixType::ControlOverExpectedP1
            )
    ...
    #[test]
    fn exp_log_observed_expected_preserves_double_intermediate_precision() {
        let expected = ExpectedValueVector {
            key: ExpectedValueKey {
                normalization: "NONE".to_owned(),
                unit: MatrixUnit::BasePairs,
        ...
        // Fixed JDK 25 fixture for:
        // (float) Math.exp(Math.log((float) (9.25f + 1f))
        //     / Math.log((float) (2.5f + 1f)))
        // Keeping the raw bits independent of the Rust expression prevents
        // this test from merely mirroring the production implementation.
        let score = exp_log_observed_expected(record, &expected, 1, 0, 0).unwrap();
        assert_eq!(score.to_bits(), 0x40cd_1731);
    }
